        ...
        <div class="infobox halfrow">
            ...
            <table style="width: 100%">
                ...
                <tr>
                    ...
                    <td class="value">
                        <button id="info_blocked_ads"><?php echo $user_spams; ?></button>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">&nbsp;</td>
                ...
                <?php if (isset($item['fk_i_user_id'])) { ?>
                    <tr>
                        ...
                        <td class="value"><?php echo '<a href="' . osc_admin_base_url(true)
                                . '?page=users&action=edit&id=' . $item['fk_i_user_id'] . '">' . $item['fk_i_user_id']
                                . '</a>'; ?></td>
                    </tr>
                <?php } ?>
                <?php if (isset($user['s_name'])) { ?>
                    <tr>
                        ...
                        <td class="value"><?php echo '<a href="' . osc_admin_base_url(true)
                                . '?page=users&action=edit&id=' . $item['fk_i_user_id'] . '">' . $user['s_name']
                                . '</a>'; ?></td>
                    </tr>
                <?php } ?>
                <?php if (isset($item['s_user_mail'])) { ?>
                    <tr>
                        ...
                        <td class="value"><?php echo '<a href="' . osc_admin_base_url(true)
                                . '?page=users&action=edit&id=' . $item['fk_i_user_id'] . '">' . $item['s_user_mail']
                                . '</a>'; ?></td>
                    </tr>
                <?php } ?>
                ...
                <tr>
                    ...
                    <td class="value"><?php if (isset($item['dt_date'])) {
                            echo $item['dt_date'];
                        } ?></td>
                </tr>
            </table>
            ...
            <div class="actionbuttons">
                <a class="btn btn-submit" onclick="return confirm('Are you sure you want to activate this item?');"
                   href="<?php echo osc_admin_base_url(true) . '?page=items&spam=activate&item='
                       . $id; ?>"><?php _e('Activate Ad', 'spamprotection'); ?></a>
                <a class="btn" onclick="return confirm('Are you sure you want to delete this item?');"
                   href="<?php list($csrfname, $csrftoken) = osc_csrfguard_generate_token();
                   echo osc_admin_base_url(true) . '?page=items&action=delete&id[]=' . $id . '&CSRFName=' . $csrfname
                       . '&CSRFToken=' . $csrftoken; ?>"><?php _e('Delete Ad', 'spamprotection'); ?></a>
                <div style="clear: both; margin: 15px 0;"></div>
                <a class="btn btn-red" onclick="return confirm('Are you sure you want to block this user?');"
                   href="<?php echo osc_admin_base_url(true) . '?page=items&spam=block&mail='
                       . $item['s_user_mail']; ?>"><?php _e('Block User', 'spamprotection'); ?></a>
                <a class="btn btn-red" onclick="return confirm('Are you sure you want to ban this user completely?');"
                   href="<?php echo osc_admin_base_url(true) . '?page=items&addIpBan=' . $id; ?>"><?php _e('IP Ban',
                        'spamprotection'); ?></a>
            </div>
        </div>
        <div class="infobox halfrow">
            <div class="container">
                ...
                <ul class="langtabs">
                    <?php foreach (osc_get_locales() as $k => $v) {
                        ...
                        ?>
                        <li class="langtab-link<?php echo $current; ?>" data-tab="tab-<?php echo $v['pk_c_code']; ?>">
                            <a><?php echo $v['s_name']; ?></a></li>
                    <?php } ?>
                </ul>
                    ...
                    if (is_array($item_spam) || is_object($item_spam)) {
                        foreach ($item_spam as $ik => $iv) {
                            if ($v['pk_c_code'] == $iv['fk_c_locale_code']) { ?>
                                ...
                                <div id="tab-<?php echo $v['pk_c_code']; ?>"
                                     class="langtab-content<?php echo $current; ?>">
                                    <div class="form-group">
                                        <h3><?php echo osc_esc_html($iv['s_title']); ?></h3>
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label"><?php _e('Description',
                                                'spamprotection'); ?></label><br/>
                                        <textarea id="descriptionCode" class="form-control"
                                                  readonly="readonly"><?php echo osc_esc_html($iv['s_description']); ?></textarea>
                ...
                foreach ($item_spams as $k => $v) { ?>
                    <tr>
                        <td><?php
                            ...
                            if ($vItem) {
                                echo '<a href="' . osc_admin_base_url(true) . '?page=items&action=item_edit&id='
                                    . $v['fk_i_item_id'] . '">' . $v['fk_i_item_id'] . '</a>';
                            } else {
                                echo $v['fk_i_item_id'];
                            }
                            ?></td>
