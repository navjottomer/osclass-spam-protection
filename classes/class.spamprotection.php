...
class spam_prot extends DAO
{
    ...
    public function _install()
    {
        ...
        $opts = self::newInstance()->_opt();
        $pref = self::newInstance()->_sect();
        foreach ($opts AS $k => $v) {
    ...
    public function _correctDatabase()
    {
        $this->dao->select('*');
        ...
        if ($result && $result->numRows() > 0) {
            ...
            foreach ($data as $user) {
                if (isset($user['i_reputation'])) {
                    $this->dao->insert($this->_table_sp_users, array(
                        'pk_i_id'      => $user['pk_i_id'],
                        'i_reputation' => $user['i_reputation'],
                        's_reputation' => $user['s_reputation']
                    ));
                }
            }
        }
     ...
     * @return array|mixed
     */
    public function _opt($key = false)
    {
        $opts = array(
    ...
    public function _admin_menu_draw()
    {
        ...
        if ($count > 0 || $comments > 0 || $contacts > 0 || $bans > 0) {
            if ($sidebar == '1') {
                osc_add_admin_submenu_divider('spamprotection', __('Actions', 'spamprotection'),
                    'spamprotection_separator', 'administrator');
            }
        }
        if ($count > 0) {
            if ($sidebar == '1') {
                osc_add_admin_submenu_page('spamprotection', sprintf(__('%s Spam found', 'spamprotection'), $count),
                    osc_admin_base_url(true) . '?page=items&b_spam=1', 'spamprotection_spam_found', 'administrator');
            }
            ...
            if ($topbar == '1') {
                AdminToolbar::newInstance()->add_menu(array(
                    ...
                    'title'  => ($toptype === 'text'
                        ? '<i class="circle circle-gray">' . $count . '</i>' . __('Spam ad',
                            'spamprotection')
                        : '<i class="sp-alert ads' . ($pulse ? ' pulse' : ' highlight') . '"><span>' . $count
                        . '</span></i>'),
                    'href'   => osc_admin_base_url(true) . '?page=items&b_spam=1',
                    'meta'   => ($toptype === 'text' ? array('class' => 'action-btn action-btn-black')
                        : array('style' => 'min-width: 0; padding: 0;')),
                    'target' => '_self'
                ));
            }
        }
        if ($comments > 0) {
            if ($sidebar == '1') {
                osc_add_admin_submenu_page('spamprotection',
                    sprintf(__('%s Spam comment found', 'spamprotection'), $comments),
                    osc_admin_render_plugin_url(osc_plugin_folder(__DIR__) . 'admin/comments_check.php'),
                    'spamprotection_spamcomment_found', 'administrator');
            }
            ...
            if ($topbar == '1') {
                AdminToolbar::newInstance()->add_menu(array(
                    ...
                    'title'  => ($toptype === 'text'
                        ? '<i class="circle circle-gray">' . $comments . '</i>'
                        . __('Spam comment', 'spamprotection')
                        : '<i class="sp-alert comments' . ($pulse ? ' pulse' : ' highlight') . '"><span>' . $comments
                        . '</span></i>'),
                    'href'   => osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                        . 'admin/comments_check.php'),
                    'meta'   => ($toptype === 'text' ? array('class' => 'action-btn action-btn-black')
                        : array('style' => 'min-width: 0; padding: 0;')),
                    'target' => '_self'
                ));
        ...
        if ($contacts > 0) {
            if ($sidebar == '1') {
                osc_add_admin_submenu_page('spamprotection',
                    sprintf(__('%s Spam Contact Mail found', 'spamprotection'), $contacts),
                    osc_admin_render_plugin_url(osc_plugin_folder(__DIR__) . 'admin/contact_check.php'),
                    'spamprotection_spamcontact_found', 'administrator');
            }
            ...
            if ($topbar == '1') {
                AdminToolbar::newInstance()->add_menu(array(
                    ...
                    'title'  => ($toptype === 'text'
                        ? '<i class="circle circle-gray">' . $contacts . '</i>'
                        . __('Spam Mail', 'spamprotection')
                        : '<i class="sp-alert mails' . ($pulse ? ' pulse' : ' highlight') . '"><span>' . $contacts
                        . '</span></i>'),
                    'href'   => osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                        . 'admin/contact_check.php'),
                    'meta'   => ($toptype === 'text' ? array('class' => 'action-btn action-btn-black')
                        : array('style' => 'min-width: 0; padding: 0;')),
                    'target' => '_self'
                ));
        ...
        if ($bans > 0) {
            if ($sidebar == '1') {
                osc_add_admin_submenu_page('spamprotection', sprintf(__('%s Banned user', 'spamprotection'), $bans),
                    osc_admin_render_plugin_url(osc_plugin_folder(__DIR__) . 'admin/ban_log.php'),
                    'spamprotection_bans_found', 'administrator');
            }
            ...
            if ($topbar == '1') {
                AdminToolbar::newInstance()->add_menu(array(
                    ...
                    'title'  => ($toptype === 'text'
                        ? '<i class="circle circle-gray">' . $bans . '</i>'
                        . __('Banned user', 'spamprotection')
                        : '<i class="sp-alert bans' . ($pulse ? ' pulse' : ' highlight') . '"><span>' . $bans
                        . '</span></i>'),
                    'href'   => osc_admin_render_plugin_url(osc_plugin_folder(__DIR__) . 'admin/ban_log.php'),
                    'meta'   => ($toptype === 'text' ? array('class' => 'action-btn action-btn-black')
                        : array('style' => 'min-width: 0; padding: 0;')),
                    'target' => '_self'
                ));
    ...
    {
        if ($table === 't_item') {
            $table = $this->_table_item;
        } elseif ($table === 't_user') {
            $table = $this->_table_user;
        } elseif ($table === 't_bans') {
            $table = $this->_table_bans;
        } elseif ($table === 't_sp_items') {
            $table = $this->_table_sp_items;
        } elseif ($table === 't_comment') {
            $table = $this->_table_comment;
        } elseif ($table === 't_sp_ban_log') {
            $table = $this->_table_sp_ban_log;
        } elseif ($table === 't_sp_contacts') {
            $table = $this->_table_sp_contacts;
        }
    ...
    public function _admin_topicon()
    {
        $count    = $this->_countRows('t_item',
            array(array('key' => 'b_spam', 'value' => '1'), array('key' => 'b_active', 'value' => '0')));
        $comments = $this->_countRows('t_comment', array('key' => 'b_spam', 'value' => '1'));
        ...
        if ($count > 0) {
            echo '<li><a href="' . osc_admin_base_url(true) . '?page=items&b_spam=1"><i class="circle circle-gray">'
                . $count . '</i>' . __('Spam ads', 'spamprotection') . '</a></li>';
        }
        if ($comments > 0) {
            echo '<li><a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                    . 'admin/comments_check.php') . '"><i class="circle circle-gray">' . $comments . '</i>'
                . __('Spam comments', 'spamprotection') . '</a></li>';
        }
        if ($contacts > 0) {
            echo '<li><a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                    . 'admin/contact_check.php') . '"><i class="circle circle-gray">' . $contacts . '</i>'
                . __('Spam mails', 'spamprotection') . '</a></li>';
        }
        if ($bans > 0) {
            echo '<li><a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                    . 'admin/ban_log.php') . '"><i class="circle circle-gray">' . $bans . '</i>' . __('Bans found',
                    'spamprotection') . '</a></li>';
        }

        echo '
                    <li><a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                . 'admin/main.php&tab=settings') . '"' . ($count > 0 || $comments > 0 || $contacts > 0 || $bans > 0
                ? ' style="border-top: 1px solid #fff;"' : '') . '>&raquo; ' . __('Dashboard', 'spamprotection') . '</a></li>
                    <li><a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                . 'admin/main.php&tab=sp_config') . '">&raquo; ' . __('Settings', 'spamprotection') . '</a></li>
                    <li><a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                . 'admin/main.php&tab=sp_config&subtab=log') . '">&raquo; ' . __('Global Log', 'spamprotection') . '</a></li>
                    <li><a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__)
                . 'admin/main.php&tab=sp_help') . '">&raquo; ' . __('Help', 'spamprotection') . '</a></li>
                </ul>
            </nav>
    ...
    public function _getResult($table, $where = false, $orderBy = false, $orderDir = 'DESC')
    {
        ...
        if ($table === 't_item') {
            $table = $this->_table_item;
        } elseif ($table === 't_item_description') {
            $table = $this->_table_desc;
        } elseif ($table === 't_user') {
            $table = $this->_table_user;
        } elseif ($table === 't_comment') {
            $table = $this->_table_comment;
            ...
            $table = $this->_table_sp_items;
        } elseif ($table === 't_sp_users') {
            $table = $this->_table_sp_users;
        } elseif ($table === 't_sp_comments') {
            $table = $this->_table_sp_comments;
        } elseif ($table === 't_sp_contacts') {
            $table = $this->_table_sp_contacts;
        }
     ...
     * @return bool
     */
    public function _saveSettings($params, $type = false)
    {
        ...
        $sort           = array(
            ...
            'comment_blocked',
            'comment_blocked_tld',
            'sp_comment_stopwords',
            'contact_blocked',
            'contact_blocked_tld',
            'sp_contact_stopwords'
        );
        ...
        $pluginsettings = array(
            ...
            'sp_activate_menu',
            'sp_activate_topicon',
            'sp_activate_pulsemenu',
            'sp_menu_after',
            'sp_topicon_position',
            'sp_update_check',
            'sp_theme',
            'sp_globallog_activate',
            'sp_globallog_limit',
            'sp_globallog_lifetime'
        );
        ...
        if (!empty($params['trusted'])) {
            ...
            foreach ($params['trusted'] as $k => $v) {
                $value = serialize($v);
                $this->dao->insert($this->_table_sp_users,
                    array('pk_i_id' => $k, 'i_reputation' => '2', 's_reputation' => $value));
            }
        }
        ...
        if (!empty($params['bad'])) {
            ...
            foreach ($params['bad'] as $k => $v) {
                $value = serialize($v);
                $this->dao->insert($this->_table_sp_users,
                    array('pk_i_id' => $k, 'i_reputation' => '1', 's_reputation' => $value));
            }
        }
            ...
            if (!in_array($k, $mailtemplates) && !in_array($k, $pluginsettings) && !in_array($k, $forbidden)) {
                if (isset($params[$k])) {
                    ...
                    if (in_array($k, $serialize)) {
                        $value = serialize(array_filter($value, static function ($value) { return $value !== ''; }));
                    }
     ...
     * @param $reason
     */
    public function _markAsSpam($data, $reason)
    {
        $this->dao->update($this->_table_item, array('b_enabled' => '0', 'b_active' => '0', 'b_spam' => '1'),
            array('pk_i_id' => $data['pk_i_id']));
     ...
     * @param $reason
     */
    public function _markCommentAsSpam($data, $reason)
    {
        $this->dao->update($this->_table_comment, array('b_enabled' => '0', 'b_active' => '0', 'b_spam' => '1'),
            array('pk_i_id' => $data['pk_i_id']));
     ...
     * @param $token
     */
    public function _markContactAsSpam($data, $reason, $token)
    {
     ...
     * @return bool
     */
    public function _spamAction($type, $id, $ip = false)
    {

        if ($type === 'activate') {
            $this->dao->update($this->_table_item, array('b_spam' => '0', 'b_enabled' => '1', 'b_active' => '1'),
                array('pk_i_id' => $id));
        } elseif ($type === 'block') {
     ...
     * @return bool
     */
    public function _addBanLog($type, $reason, $email = false, $ip = false, $mode = 'user')
    {
        ...
        } elseif ($email && $mode === 'admin') {
            $user = Admin::newInstance()->findByUsername($email);
        }
        ...
        if ($reason === 'falselogin') {
            $reason_sql .= '&nbsp;' . __('too many false logins', 'spamprotection');
        } elseif ($reason === 'spam') {
            $reason_sql .= '&nbsp;' . __('spam ads', 'spamprotection');
        }

        if ($this->dao->insert($this->_table_sp_ban_log, array(
            'i_user_id'    => (isset($user['pk_i_id']) ? $user['pk_i_id'] : false),
            's_user_email' => $email,
            's_user_ip'    => $ip,
            's_reason'     => $reason_sql
        ))
     ...
     * @return bool
     */
    public function _spamActionComments($type, $id)
    {
        ...
        if ($type === 'activate') {
            ...
            $this->dao->update($this->_table_comment, array('b_active' => '1', 'b_enabled' => '1', 'b_spam' => '0'),
                array('pk_i_id' => $comment['fk_i_comment_id']));
            ...
            if (!in_array($comment['s_user_mail'], $blocked)) {
                $blocked[] = $comment['s_user_mail'];
            }
     ...
     * @param $done
     */
    public function _addGlobalLog($action, $account, $done)
    {
        $activate = $this->_get('sp_globallog_activate');
        if (isset($activate) && $activate = '1') {
            $this->dao->insert($this->_table_sp_globallog,
                array('s_reason' => $action, 's_account' => $account, 's_done' => $done));
        }
    }
     ...
     * @return bool
     */
    public function _spamActionContacts($type, $id)
    {
        $admin   = Admin::newInstance()->findByPrimaryKey(osc_logged_admin_id());
        ...
        if ($type === 'forward') {
            ...
            if (!in_array($contact['s_user_mail'], $blocked)) {
                $blocked[] = $contact['s_user_mail'];
            }
            $blocked = implode(',', array_filter($blocked));
     ...
     * @return bool
     */
    public function _forwardMail($data, $itemid)
    {
        $id          = $itemid;
        ...
        $title = osc_apply_filter('email_item_inquiry_title_after', osc_mailBeauty(osc_apply_filter('email_title',
            osc_apply_filter('email_item_inquiry_title', $content['s_title'], $data)), $words), $data);
        $body  = osc_apply_filter('email_item_inquiry_description_after',
            osc_mailBeauty(osc_apply_filter('email_description',
                osc_apply_filter('email_item_inquiry_description', $content['s_text'], $data)), $words), $data);
     ...
     * @return array|bool
     */
    public function _checkForSpam($item, $type = 'mail')
    {
        $params = Params::getParamsAsArray();
        if ($type === 'mail') {
            $user = $item['s_contact_email'];
        } else {
        ...
        if ($this->_get('sp_blocked') == '1' && !empty($blocked) && $this->_checkBlocked($item['s_contact_email'])) {
            $this->_addGlobalLog('Blocked email address found while posting ad for itemID: ' . $item['pk_i_id'],
                $item['s_contact_email'], 'System');
        ...
        if ($this->_get('sp_blocked_tld') == '1' && !empty($blocked_tld)
            && $this->_checkBlockedTLD($item['s_contact_email'])
        ) {
            $this->_addGlobalLog('Blocked email hoster found while posting ad for itemID: ' . $item['pk_i_id'],
                $item['s_contact_email'], 'System');
        ...
        if ($this->_get('sp_mxr') == '1') {
            ...
            if ($check['status'] == false) {
                $this->_addGlobalLog('No MX Record found while posting ad for itemID: ' . $item['pk_i_id'],
                    $item['s_contact_email'], 'System');
            ...
            while (osc_has_web_enabled_locales()) {
                foreach ($items as $ik => $iv) {
                    ...
                    if ($checkTitle && $item['pk_i_id'] != $iv['pk_i_id']) {
                        $this->_addGlobalLog('Duplicate title for itemID ' . $iv['pk_i_id'] . ' detected: '
                            . (is_numeric($checkTitle) ? 'Similarity: ' . $checkTitle . '%' : ''), '', 'System');

                        return array(
                            'params' => $item,
                            'reason' => '<a href="' . osc_admin_base_url(true) . '?page=items&action=item_edit&id='
                                . $iv['pk_i_id'] . '">Duplicate title found for ItemID: ' . $iv['pk_i_id'] . '. '
                                . (is_numeric($checkTitle) ? 'Similarity: ' . $checkTitle . '%' : '') . '</a>'
                        );
                    }

                    if ($this->_get('sp_duplicates') == '1') {
                        $checkDescription =
                            $this->_checkForDescription($iv['pk_i_id'], $item['fk_i_item_id'], osc_locale_code());
                        if ($checkDescription && $item['pk_i_id'] != $iv['pk_i_id']) {
                            $this->_addGlobalLog('Duplicate description for itemID ' . $iv['pk_i_id'] . ' detected: '
                                . (is_numeric($checkDescription) ? 'Similarity: ' . $checkDescription . '%' : ''), '',
                                'System');

                            return array(
                                'params' => $item,
                                'reason' => '<a href="' . osc_admin_base_url(true) . '?page=items&action=item_edit&id='
                                    . $iv['pk_i_id'] . '">Duplicate description found for ItemID: ' . $iv['pk_i_id']
                                    . '. ' . (is_numeric($checkDescription) ? 'Similarity: ' . $checkDescription . '%'
                                        : '') . '</a>'
                            );
                        }
     ...
     * @return bool
     */
    public function _checkHoneypot($params)
    {
     ...
     * @return bool
     */
    public function _checkBlocked($mail, $type = false)
    {
        if ($type === 'comment') {
     ...
     * @return bool
     */
    public function _checkBlockedTLD($mail, $type = false)
    {
        if ($type === 'comment') {
     ...
     * @return array
     */
    public function _validateMail($email, $test_mx = true)
    {
        $email = trim($email);
     ...
     * @return bool
     */
    public function _checkTOR($action = false)
    {
        $valid = false;
        $ip    = self::newInstance()->_IpUserLogin();
        if (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
            $file = fopen(osc_plugin_path('spamprotection/tor_nodes.txt'), 'rb');
            while (($buffer = fgets($file)) !== false) {
                if (strpos($buffer, $ip) !== false) {
                    ...
                    ob_get_clean();
                    osc_add_flash_error_message(__('Posting a new ad is not allowed because of using the TOR Network. Please disable TOR and try again.'));
     ...
     * @return bool|mixed
     */
    public function _checkStopwords($item)
    {
     ...
     * @return bool
     */
    public function _getItemsByUser($user)
    {
        $this->dao->select('*');
        ...
        if (is_numeric($user)) {
            ...
            $this->dao->where('s_contact_email', $user);
        }
     ...
     * @return bool
     */
    public function _getItemsByAll()
    {
     ...
     * @return bool|string
     */
    public function _checkForTitle($item1, $item2, $locale)
    {
        if ($item1 == $item2) {
            return false;
        }
        ...
        if ($this->_get('sp_duplicate_type') == '1') {
            ...
            $compare = ($this->_get('sp_duplicate_perc') ?: 0);
        ...
        return strtolower(trim(@$check1['s_title'])) === strtolower(trim(@$check2['s_title']));
    }
     ...
     * @return bool
     */
    public function _getItemData($item, $locale)
    {
     ...
     * @return bool|string
     */
    public function _checkForDescription($item1, $item2, $locale)
    {
        $check1 = $this->_getItemData($item1, $locale);
        $check2 = $this->_getItemData($item2, $locale);
        ...
        return md5(strtolower(trim(@$check1['s_description']))) === md5(strtolower(trim(@$check2['s_description'])));
    }
     ...
     * @return array|bool
     */
    public function _checkComment($id)
    {
     ...
     * @return bool
     */
    public function _getComment($id)
    {
        $this->dao->select('*');
     ...
     * @return bool|mixed
     */
    public function _checkStopwordsComments($comment)
    {
     ...
     * @return array|bool
     */
    public function _checkContact($data)
    {
        $params = Params::getParamsAsArray();
        ...
        // Check for Honeypot
        if (($this->_get('sp_contact_honeypot') == '1') && $this->_checkHoneypotContact($params)) {
        ...
        if ($this->_get('sp_contact_blocked') == '1' && !empty($blocked)
            && $this->_checkBlocked($params['yourEmail'], 'contact')
        ) {
        ...
        if ($this->_get('sp_contact_blocked_tld') == '1' && !empty($blocked_tld)
            && $this->_checkBlockedTLD($params['yourEmail'], 'contact')
        ) {
     ...
     * @return bool
     */
    public function _checkHoneypotContact($params)
    {
     ...
     * @return bool|mixed
     */
    public function _checkStopwordsContact($comment)
    {
     ...
     * @return bool
     */
    public function _refreshTOR()
    {
        $file = osc_plugin_path('spamprotection/tor_nodes.txt');
        $data =
            file_get_contents('https://check.torproject.org/cgi-bin/TorBulkExitList.py?ip=' . $_SERVER['SERVER_ADDR']);
     ...
     * @return bool
     */
    public function _checkContactUser($user)
    {
        $this->dao->select('*');
        $this->dao->from($this->_table_user);
     ...
     * @return bool
     */
    public function _searchSpamContact($uniqid)
    {
        $this->dao->select('*');
     ...
     * @param string $email
     */
    public function _deleteMailByUser($id, $token, $email = '')
    {
        $contact = $this->_getRow('t_sp_contacts', array('key' => 'pk_i_id', 'value' => $id));
     ...
     * @return bool
     */
    public function _checkUserLogin($email, $password)
    {
        $user = User::newInstance()->findByEmail($email);
     ...
     * @return bool
     */
    public function _checkUserBan($email, $ip)
    {
     ...
     * @param $ip
     */
    public function _handleUserLogin($email, $ip)
    {
     ...
     * @return array|bool|mixed
     */
    public function _doIpBan($do, $ip)
    {
        $table = unserialize($this->_get('sp_ipban_table'));
        if ($do === 'add') {
            if (is_array($table)) {
     ...
     * @return bool
     */
    public function _checkAdminLogin($admin, $password)
    {
     ...
     * @return bool
     */
    public function _checkAdminBan($ip)
    {
     ...
     * @param $ip
     */
    public function _handleAdminLogin($name, $ip)
    {
        $action = $this->_get('sp_admin_login_action');
        $reason = sprintf(__('Spam Protection - Admin/Mod %s blocked in due to too many false login attempts',
            'spamprotection'), $name);
     ...
     * @return bool
     */
    public function _countLogin($search, $type = 'user', $ip = false)
    {
     ...
     * @return bool
     */
    public function _increaseUserLogin($email)
    {
        $ip = $this->_IpUserLogin();
        if ($this->dao->insert($this->_table_sp_logins,
            array('s_email' => $email, 's_ip' => $ip, 's_type' => 'user', 'dt_date_login' => time()))
        ) {
            return true;
        }
     ...
     * @return bool
     */
    public function _increaseAdminLogin($name)
    {

        $ip = $this->_IpUserLogin();
        if ($this->dao->insert($this->_table_sp_logins,
            array('s_name' => $name, 's_ip' => $ip, 's_type' => 'admin', 'dt_date_login' => time()))
        ) {
            return true;
        }
     ...
     * @param $email
     */
    public function _resetUserLogin($email)
    {
        $time = $this->_get('sp_security_login_time') * 60;
        $ip   = $this->_IpUserLogin();
     ...
     * @param $name
     */
    public function _resetAdminLogin($name)
    {
        $time = $this->_get('sp_security_login_time') * 60;
        $ip   = $this->_IpUserLogin();
        //$this->dao->delete($this->_table_bans, '`s_ip` = "'.$ip.'"');
     ...
     * @return bool
     */
    public function _unbanStopForumSpam()
    {
     ...
     * @param $id
     */
    public function _handleBanLog($action, $id)
    {
        if ($action === 'activate') {
            $log = $this->_getRow('t_sp_ban_log', array('key' => 'pk_i_id', 'value' => $id));
            if (isset($log['s_user_email'])) {
                $this->dao->update($this->_table_user, array('b_enabled' => '1'),
                    array('s_email' => $log['s_user_email']));
     ...
     * @return bool
     */
    public function _informUser($search, $target = 'user')
    {
        $ip   = $this->_IpUserLogin();
        ...
        if ($target === 'user') {
            ...
            $content[] = array(
                ...
                $time,
                $ip,
                '<a href="' . osc_base_url(true) . '?page=sp_activate_account&email=' . $search . '&token='
                . md5((isset($user['s_secret']) ? $user['s_secret'] : '')) . '">Click here</a>',
                '<a href="' . osc_recover_user_password_url() . '">Click here</a>',
                '<a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__) . 'admin/ban_log.php')
                . '">Ban List</a>'
            );
        } elseif ($target === 'admin') {
            ...
            $content[] = array(
                ...
                $time,
                $ip,
                '<a href="' . osc_base_url(true) . '?page=sp_activate_account&name=' . $search . '&token='
                . md5((isset($user['s_secret']) ? $user['s_secret'] : '')) . '">Click here</a>',
     ...
     * @return string
     */
    public function _titleTemplate($target = false, $target2 = false)
    {
     ...
     * @return string
     */
    public function _mailTemplate($type = 'html', $target = false, $target2 = false)
    {
...
'
                . __('We have detected some false logins for your account {MAIL_USED} on {PAGE_NAME}. Last false login was on {MAIL_DATE} from IP {MAIL_IP}',
                    'spamprotection') . '<br /><br />
'
...
{PAGE_NAME}';
     ...
     * @return bool
     */
    public function _checkAccount($search, $type = 'user')
    {
        $this->dao->select('*');
     ...
     * @return bool
     */
    public function _testMail($target, $target2, $email)
    {
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            return false;
        }
        ...
        $plain[] = array(
            ...
            $time,
            $ip,
            osc_base_url(true) . '?page=sp_activate_account&name=' . $email . '&token=' . md5($user['s_secret']),
        ...
        $html[] = array(
            ...
            $time,
            $ip,
            '<a href="' . osc_base_url(true) . '?page=sp_activate_account&name=' . $email . '&token='
            . md5($user['s_secret']) . '">Click here</a>',
            '<a href="' . osc_admin_base_url(true) . '?page=login&action=recover">Click here</a>',
            '<a href="' . osc_admin_render_plugin_url(osc_plugin_folder(__DIR__) . 'admin/ban_log.php')
            . '">Ban List</a>'
        );
     ...
     * @param $reason
     */
    public function _addBanRule($type, $data, $reason)
    {
        if ($type === 'email') {
            $string  = 's_email';
            $string2 = 's_user_email';
        } elseif ($type === 'ip') {
            $string  = 's_ip';
            $string2 = 's_user_ip';
        }
     ...
     * @return bool
     */
    public function _checkBanRule($email = false, $ip = false)
    {
        $this->dao->select('*');
        $this->dao->from($this->_table_bans);
     ...
     * @param        $userID
     * @param string $type
     * @param string $badortrusted
     *
     * @return bool
     */
    public function _isBadOrTrusted($userID, $type = 'ads', $badortrusted = 'trusted')
    {
        ...
        if ($bt == '1') {
            ...
            if (isset($user) && !empty($user)) {
                ...
                $trusted    =
                    array('ads' => 'trustedads', 'comments' => 'trustedcomments', 'contacts' => 'trustedcontacts');
     ...
     * @return bool
     */
    public function _BadOrTrustedUsers($id)
    {
     ...
     * @return mixed
     */
    public function _userBadTrustedData($row, $user)
    {
        ...
        if (isset($user['i_reputation']) && $user['i_reputation'] == '1') {
            $rep = '
            <a href="'
                . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_security&sub=badtrusted&table=bad')
                . '">
                <i class="sp-icon thumbsdown" style="display: block; width: 32px; height: 32px; transform: scale(0.6);" title="'
                . __('This user is on your bad user list', 'spamprotection') . '"></i>
            </a>
            ';
        } elseif (isset($user['i_reputation']) && $user['i_reputation'] == '2') {
            $rep = '
            <a href="'
                . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_security&sub=badtrusted&table=trusted')
                . '">
                <i class="sp-icon thumbsup" style="display: block; width: 32px; height: 32px; transform: scale(0.6);" title="'
                . __('This user is on your trusted user list', 'spamprotection') . '"></i>
            </a>';
        }
     ...
     * @return bool
     */
    public function _searchBadOrTrustedUser($search)
    {
     ...
     * @param $user
     */
    public function _userManage($action, $user)
    {
     ...
     * @return bool
     */
    public function _userManageAjax($action, $id)
    {
     ...
     * @return array
     */
    public function _userManageLinks($options, $user)
    {
        $bot    = $this->_BadOrTrustedUsers($user['pk_i_id']);
        $return = $options;

        if (!isset($bot['i_reputation']) || $bot['i_reputation'] == '0') {
            $return[] = '<a href="' . osc_admin_base_url(true) . '?page=users&adduser=2&user=' . $user['pk_i_id'] . '">'
                . __('Trusted User', 'spamprotection') . '</a>';
            $return[] = '<a href="' . osc_admin_base_url(true) . '?page=users&adduser=1&user=' . $user['pk_i_id'] . '">'
                . __('Bad User', 'spamprotection') . '</a>';
        } elseif ($bot['i_reputation'] == '1') {
            $return[] = '<a href="' . osc_admin_base_url(true) . '?page=users&adduser=2&user=' . $user['pk_i_id'] . '">'
                . __('Trusted User', 'spamprotection') . '</a>';
            $return[] =
                '<a href="' . osc_admin_base_url(true) . '?page=users&adduser=remove&user=' . $user['pk_i_id'] . '">'
                . __('Remove Bad User', 'spamprotection') . '</a>';
        } elseif ($bot['i_reputation'] == '2') {
            $return[] = '<a href="' . osc_admin_base_url(true) . '?page=users&adduser=1&user=' . $user['pk_i_id'] . '">'
                . __('Bad User', 'spamprotection') . '</a>';
            $return[] =
                '<a href="' . osc_admin_base_url(true) . '?page=users&adduser=remove&user=' . $user['pk_i_id'] . '">'
                . __('Remove Trusted User', 'spamprotection') . '</a>';
        }
     ...
     * @return array|bool|mixed
     */
    public function _listIpBanTable()
    {
     ...
     * @return array|bool
     */
    public function _userRowIpBan($options, $user)
    {
        $return = false;
        if (isset($user['s_access_ip'])) {
            $return   = $options;
            $return[] = '<a href="' . osc_admin_base_url(true) . '?page=users&addIpBan=' . $user['s_access_ip'] . '">'
                . __('Add IP Ban', 'spamprotection') . '</a>';
        }
     ...
     * @param $id
     */
    public function _deleteComment($id)
    {
     ...
     * @return bool
     */
    public function _readComment($id)
    {
        $this->dao->select('*');
        $this->dao->from($this->_table_comment);
     ...
     * @param $error
     */
    public function _upgradeDatabaseInfo($error)
    {
        $params = Params::getParamsAsArray();
            ...
            <div id="spamprot_upgrade_overlay" style="display: none;">
                <div id="spamprot_upgrade" style="display: none;">
                    ...
                    <div id="spamprot_upgrade_info">
                        ...
                        ' . $error . '    
                        <p>'
            . __('Since you have upgraded this plugin, the database need an upgrade also.<br /><strong>To upgrade it now, press the button below.</strong>',
                'spamprotection') . '</p>    
                        <p>'
            . __('If you want to backup your data before, you can use the export function.<br />You will find an import function in the plugin settings.',
                'spamprotection') . '</p>    
                        <p>'
            . __('Of course you can do the upgrade manually.<br /><em>For this case you will find the database tables here</em>',
                'spamprotection') . '</p>
                        
                        <div style="height: 180px; text-align: left; margin: 15px 0;">
                            <div id="spamprot_upgrade_tables">
                                <small><strong><textarea style="width: 100%; height: 100%; resize: none; overflow: overlay;">'
            . $tables . '</textarea></strong></small>
                            </div>                            
                        </div>                            
                    </div>
     ...
     * @return string
     */
    public function _export($type = 'database')
    {

        if ($type === 'database') {
            $xmlFile  = osc_plugin_path(dirname(__DIR__)) . '/export/database.xml';
            ...
            $xmlFile  = osc_plugin_path(dirname(__DIR__)) . '/export/settings.xml';
        ...
        if (file_exists($xmlFile)) {
            ...
            if (!$dl) {
                $title = '<h1 style="display: inline-block;"><i class="sp-icon attention margin-right float-left"></i>'
                    . __('Attention', 'spamprotection') . '</h1>';
        ...
        if ($xml_file) {
            $title = '<h1 style="display: inline-block;"><i class="sp-icon attention margin-right float-left"></i>'
                . __('Success', 'spamprotection') . '</h1>';
            $error = sprintf(__('Export file %s was created succesfully', 'spamprotection'), ($type === 'database'
                ? __('for database', 'spamprotection') : __('for plugin settings', 'spamprotection')));
        } else {
            $title = '<h1 style="display: inline-block;"><i class="sp-icon attention margin-right float-left"></i>'
                . __('Attention', 'spamprotection') . '</h1>';
        ...
        $message = self::newInstance()->_showPopup($title, $error, '', 1500, false);
     ...
     * @return array|bool
     */
    public function _prepareExport($type = 'database')
    {
        if ($type === 'database') {
            $export = array(
     ...
     * @return bool
     */
    public function _selectExport($table, $where = false)
    {
        $this->dao->select('*');
        $this->dao->from($table);
     ...
     * @param mixed $class              Set own class
     * @param mixed $style              style="Your Style"
     *
     * @param       $footer             string
     * @param       $body               string
     *
     * @return string
     *
     * @example                 <h1 style="display: inline-block;">
     *                            <i class="sp-icon attention margin-right float-left"></i>Your Title
     *                          </h1>
     * @example                 <div>Your Footer</div>
     * @example                 <div style="font-size: 18px;">Your ContEnt</div>
     */
    public function _showPopup($head, $body, $footer, $time, $icon = true, $class = false, $style = false)
    {
        return '
            <div id="flash-inner" ' . $class . $style . '><div id="flash-head">' . ($icon
                ? '<div id="flash-close"><a class="ico-close">x</a></div>' : '') . '' . $head
            . '</div><div id="flash-body" style="display: inline-block;">' . $body . '</div>' . ($footer
                ? '<div id="flash-footer">' . $footer . '</div>' : '') . '</div>
            <script>$("#flashmessage").prop("id", "flash").removeClass(); $(document).ready(function(){ $("#flash").fadeIn("slow", function(){ '
            . ($time != '0' ? ' $("#flash").delay(' . $time . ').fadeOut("slow"); ' : '') . '});' . ($icon
                ? ' $(document).on("click", "#flash a.ico-close", function(event){ event.preventDefault; $("#flash").fadeOut("slow"); }); '
                : '') . '});</script>';
    }
     ...
     * @return string
     */
    public function _import($data, $type = 'server')
    {

        if ($type === 'upload') {
            $xmlFile = $data;
        } else {
            $xmlFile = osc_plugin_path(dirname(__DIR__)) . '/export/' . $data . '.xml';
        }
    ...
    public function _firstInstalled()
    {
        $info   = osc_plugin_get_info('spamprotection/index.php');
            ...
            <div id="spamprot_installed_overlay" style="display: none;">
                <div id="spamprot_installed" style="margin-top: 50px; display: none;">
                    ...
                    <div id="spamprot_installed_info">
                        ...
                        <p>'
            . __('This plugin changes the behaviour of your whole OSClass installation,<br />because of this please use it wise.',
                'spamprotection') . '</p>    
                        
                        <ul style="margin: 25px auto;width: 450px;text-align: left; font-weight: bolder;">
                            <li style="margin-bottom: 10px"><strong>&raquo;</strong> '
            . __("Don't activate functions that you don't understand. Take a look in the help to learn more about.",
                'spamprotection') . '</li>    
                            <li style="margin-bottom: 10px"><strong>&raquo;</strong> '
            . __("Don't overreact if you get a lot of spam. You just need to fine tune some options here.",
                'spamprotection') . '</li>        
                            <li style="margin-bottom: 10px"><strong>&raquo;</strong> '
            . __('This Plugin can block or ban user and admins. Keep an eye of all actions done on your page.',
                'spamprotection') . '</li>    
                        </ul>
                            
                        <p>' . __("<em>We don't want, that you ban or annoy user without reason ;)</em>",
                'spamprotection') . '</p>                            
                            
                        <br />
                        <p>'
            . __('<strong>If you are using the check for contact mails, please add some hints into your privacy policy that mails will be saved and moderated, if spam was found inside.</strong>',
                'spamprotection') . '</p>                            
                        <br />
                        <p>'
            . __('Feel free to contact me in OSClass-Forum for any kind of question.<br /><br />Best regards<br />Liath',
                'spamprotection') . '</p>
                    </div>
                    
                    <div id="spamprot_installed_buttons">
                        <a class="btn btn-green spamprot_installed_close" href="'
            . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=settings') . '">' . __('Plugin Settings',
                'spamprotection') . '</a>
                        <a class="btn btn-blue spamprot_installed_close" href="https://forums.osclass.org/plugins/(plugin)-spam-protection" target="_blank">'
            . __('Go to Forum', 'spamprotection') . '</a>
    ...
    public function _cleanDatabase()
    {
        /* clearing ads */
     ...
     * @param string $type
     */
    public function _cleanDatabaseDo($mode, $interval, $limit, $type = 'ads')
    {
     ...
     * @return bool
     */
    public function _cleanDatabaseSearch($mode, $interval, $limit, $type = 'ads')
    {
     ...
     * @return bool
     */
    public function _searchUnwantedAdmin($email)
    {
        $this->dao->select('*');
        $this->dao->from($this->_table_admin);
        $this->dao->where('s_email', $email);
     ...
     * @return bool
     */
    public function _searchUnwantedUser(
        $age = false,
        $max = false,
        $activated = false,
        $enabled = false,
        ...
        if ($activated) {
            $this->dao->where('u.b_active', 1);
        }
        if ($enabled) {
            $this->dao->where('u.b_enabled', 1);
        }
        if ($zero) {
            $this->dao->where('u.i_items < 1');
        }
     ...
     * @param $del
     */
    public function _deleteUnwantedUser($del)
    {
        if (isset($del) && is_array($del)) {
            foreach ($del as $id) {
                ...
                $admin = Admin::newInstance()->findByPrimaryKey(osc_logged_admin_id());
                $user  = User::newInstance()->findByPrimaryKey($id);
                $this->_addGlobalLog('User account deleted by Cleaner',
                    ($user['s_email'] ?: 'No Email address'), $admin['s_name']);
     ...
     * @param bool $lifetime
     */
    public function _clearGlobalLog($by, $lifetime = false)
    {
        if ($lifetime) {
     ...
     * @return bool|int
     */
    public function _countGlobalLog()
    {
        $this->dao->select('COUNT(*) as count');
     ...
     * @return bool|mixed
     */
    public function _readGlobalLog($limit, $page = false, $date = false, $search = false, $count = false)
    {
     ...
     * @return bool|null
     */
    public function _checkFiles($now = false)
    {
        /* get hashes */
        ...
        if (count($new) > 0 || count($deleted) > 0 || count($changed) > 0) {
            ...
            if ($this->_saveMasterHashes($current) && $this->_saveChangedHashes(array_filter($return))) {
                ...
                $this->_addGlobalLog('<strong>FILE SYSTEM INTEGRITY HURT, CHECK FILE MONITOR!</strong>', '',
                    'File monitor');
            } else {
                $this->_addGlobalLog('<strong>ATTENTION!!! FILE SYSTEM INTEGRITY HURT, CHANGES COULD NOT BE SAVED, CHECK MANUALLY!</strong>',
                    '', 'File monitor');
            }

            if (!$this->_emailFiles(array(
                'new'     => $return['new'],
                'changed' => $return['changed'],
                'deleted' => $return['deleted']
            ))
            ) {
                $this->_addGlobalLog('Alert email could not be sended to:', $this->_get('sp_files_alerts'),
                    'File monitor');
            }
        ...
        if (!empty($this->_loadChangedHashes())) {
     ...
     * @return bool
     */
    public function _excludedPath($fullFilename, array $pathArray)
    {
     ...
     * @return mixed
     */
    public function _loadMasterHashes()
    {
     ...
     * @return bool
     */
    public function _saveMasterHashes($hashes)
    {
     ...
     * @return bool
     */
    public function _saveChangedHashes($changes)
    {
        ...
        if (!osc_set_preference('sp_files_changedHashes', serialize(array_filter($changes)), $this->_sect(),
            'STRING')
        ) {
            return false;
        }
    ...
    public function _emailFiles($data)
    {
        ...
        $params = array(
            'from'      => osc_contact_email(),
            'from_name' => osc_page_title(),
            'subject'   => sprintf(__('%s ATTENTION, FILE SYSTEM INTEGRITY HURT!!!', 'spamprotection'),
                osc_page_title()),
            'to'        => $mail,
            'to_name'   => 'Administrator',
            'body'      => 'After checking your file system we found following issues:<br /><br />new files: '
                . count($data['new']) . '<br />changed files: ' . count($data['changed']) . '<br />deleted files: '
                . count($data['deleted']) . '<br /><br />Last scan: ' . osc_format_date(date('Y-m-d H:i:s', time()),
                    osc_date_format() . ' ' . osc_time_format())
                . '<br /><br />Please check this immediately in your dashboard for a possible break-in attempt<br /><br /><a href="'
                . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_tools&sub=files')
                . '">Open file monitor</a><br /><br />This Mail was sended from ' . $info['plugin_name'],
            'alt_body'  => "After checking your file system we found following issues:\n\nnew files: "
                . count($data['new']) . "\nchanged files: " . count($data['changed']) . "\ndeleted files: "
                . count($data['deleted']) . "\n\nLast scan: " . osc_format_date(date('Y-m-d H:i:s', time()),
                    osc_date_format() . ' ' . osc_time_format())
    ...
    public function _listFilesChanges()
    {
        $hashes = $this->_loadChangedHashes();
        ...
        if ($this->_hasFilesChanges($hashes)) {
            ...
            $list = '<a class="btn btn-green btn-success" style="float: right;" href="'
                . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_tools&sub=files&files=clear')
                . '">Clear list</a><br />';
            ...
            foreach ($hashes as $type => $hash) {
                ...
                if (is_array($hash) && count($hash) > 0 && $type !== 'changes') {
                    ...
                    foreach ($hash as $key => $val) {
                        ...
                        $size = ($is_array ? $this->_sizeFiles(@$val['size'][@$k], @$changes[@$key]['size'][@$k])
                            : $this->_sizeFiles(@$val['size'], @$changes[@$key]['size']));
                        $time = ($is_array
                            ? osc_format_date(date('Y-m-d H:i:s', @$changes[@$key]['time'][$k]),
                                osc_date_format() . ' ' . osc_time_format())
                            : osc_format_date(date('Y-m-d H:i:s', @$changes[@$key]['time']),
                                osc_date_format() . ' ' . osc_time_format()));
        ...
        <a class="btn btn-info" style="padding: 5px;" href="'
            . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_tools&sub=files&files=check') . '">'
            . __('Check Now', 'spamprotection') . '</a>
        ';
    }
...
*/
    ...
    public function _sizeFiles($size, $compare)
    {
        ...
        return sprintf("%.{$decimals}f", $size / (1024 ** $calc)) . @$sz[$calc - 1] . 'B';
    }
    ...
    public function _showAlertFiles()
    {
        ...
        if (!empty($changed)) {
            ...
            <div id="alertFilesOuter">
                <div id="alertFilesInner">
                    ...
                    <a class="btn btn-green btn-success" href="'
                . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_tools&sub=files') . '">Go to file monitor</a>
                </div>
            </div>
            <script>
            $(document).ready(function(){
    ...
    public function _clearAlertFiles()
    {
        if (!osc_delete_preference('sp_files_changedHashes', $this->_sect())) {
            return false;
        }

        header('Location: ' . osc_admin_base_url(true)
            . '?page=plugins&action=renderplugin&file=spamprotection/admin/main.php&tab=sp_tools&sub=files');
        exit;
    }
}
