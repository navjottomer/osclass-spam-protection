...
if (!empty($do) && !empty($ip)) {
    ...
    if (is_array($ips)) {
        foreach ($ips as $k => $v) {
            echo '<tr><td><a class="deleteIpBan" href="'
                . osc_ajax_plugin_url('spamprotection/functions/ipban.php&do=delete') . '" data-ip="' . $k
                . '"><i class="sp-icon delete xs"></i></a></td><td>' . $k . '</td><td>' . date('d.m.Y H:i:s', $v)
                . '</td></tr>';
        }
    } else {
    ...
    if (is_writable(osc_base_path())) {
        echo '
            <div id="flash">' . spam_prot::newInstance()->_showPopup(
                ...
                . __('<strong>Path writable.</strong>',
                    'spamprotection') . '</h1>',
                '<span><a id="createFileNow" class="btn btn-green" href="'
                . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_security&sub=ipban&createFileNow=1')
                . '">Create now</a></span>',
        ...
        echo '
            <div id="flash">' . spam_prot::newInstance()->_showPopup(
                '<h1 style="display: inline-block;"><i class="sp-icon attention margin-right float-left"></i>'
                . __('<strong>Path NOT writable.</strong>',
                    'spamprotection') . '</h1>',
                '
