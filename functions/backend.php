...
function sprot_admin_page_header($message = false)
{
    ...
    echo '
    <h1 style="display: inline-block;font-size: 20px;line-height: 50px; margin-top: -10px;">' . ($message
            ?: '<i class="sp_header_icon" style="margin: 0;"></i>' . sprintf(__('Anti Spam & Protection System',
                    'spamprotection') . ' v' . $info['version'])) . '</h1>
    <div style="line-height: 36px; float: right;">
        ...
        . __('This plugin is and will always be free of charge! If you found any errors, please contact me in OSClass Forum to solve your problems before rating this plugin.',
            'spamprotection') . '</p>    
                <p style="line-height: 20px;">'
        . __('If you are happy with this plugin and love to use it, please rate it now on OSClass Market!',
            'spamprotection') . '</p>    
...
function sprot_style_admin()
{
    ...
    osc_enqueue_style('spam_protection-upgrade_css',
        osc_plugin_url('spamprotection/assets/css/admin_general.css') . 'admin_general.css?' . time());
    osc_register_script('spam_protection-upgrade_js',
        osc_plugin_url('spamprotection/assets/js/admin_general.js') . 'admin_general.js?' . time(), array('jquery'));
    ...
    if (isset($params['file'])) {
        ...
        if ($plugin[0] === 'spamprotection') {
            ...
            osc_enqueue_style('spam_protection-styles_admin',
                osc_plugin_url('spamprotection/assets/css/admin_plugin.css') . 'admin_plugin.css?' . time());

            osc_register_script('spam_protection-admin',
                osc_plugin_url('spamprotection/assets/js/admin_plugin.js') . 'admin_plugin.js?' . time(),
                array('jquery'));
...
function sprot_style_admin_footer()
{
    ...
    $items    = spam_prot::newInstance()->_countRows('t_item',
    ...
    if ($topicon == '1') {
        ...
        $(document).ready(function(){
            ...
            ' . ($items > 0 || $comments > 0 || $contacts > 0 || $bans > 0
                ? '$("#osc_toolbar_spamprotection > a > i").addClass("highlight");' : '') . '
    ...
    if ($menu == '1' && $order !== 'anywhere') {
        ...
        $(document).ready(function(){
            ...
            ' . ($items > 0 || $comments > 0 || $contacts > 0 || $bans > 0
                ? '$("ul.oscmenu li#menu_spamprotection a").addClass("highlight");' : '') . '
...
function sprot_admin_menu_init()
{
    ...
    AdminMenu::newInstance()->add_menu_tools(__('Anti Spam & Protection', 'spamprotection'),
        osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=settings'), 'sprot_admin_settings',
        'administrator');

    if ($sidebar == '1') {
        osc_add_admin_menu_page(__('Anti Spam & Protection', 'spamprotection'),
            osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=settings'), 'spamprotection',
            'administrator');
        osc_add_admin_submenu_divider('spamprotection', __('Pages', 'spamprotection'), 'spamprotection_divider',
            'administrator');
        osc_add_admin_submenu_page('spamprotection', __('Dashboard', 'spamprotection'),
            osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=settings'), 'spamprotection_dashboard',
            'administrator');
        osc_add_admin_submenu_page('spamprotection', __('Settings', 'spamprotection'),
            osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_config'), 'spamprotection_settings',
            'administrator');
        osc_add_admin_submenu_page('spamprotection', __('Help', 'spamprotection'),
            osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=sp_help'), 'spamprotection_help',
            'administrator');
    }
}
...
function sprot_admin_menu()
{
    ...
    <ul>
        <li><a href="' . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=settings') . '">&raquo; '
        . __('Settings', 'spamprotection') . '</a></li>
        <li><a href="' . osc_admin_render_plugin_url('spamprotection/admin/main.php&tab=help') . '">&raquo; '
        . __('Help', 'spamprotection') . '</a></li>
    </ul>';
}
...
function sp_compare_items($options, $item)
{
    ...
    if ($item['b_spam'] == '1' && $item['b_active'] == '0') {
        $return[] =
            '<a href="' . osc_admin_render_plugin_url('spamprotection/admin/check.php&itemid=' . $item['pk_i_id'])
            . '">' . __('Check Spam', 'spamprotection') . '</a>';
    }
...
function sp_check_admin_login()
{
    ...
    if ($admin && is_array($admin)) {
        $admin_name  = $admin['s_username'];
        $admin_email = $admin['s_email'];
        ...
        if (!empty($data_token) || spam_prot::newInstance()->_checkAdminBan($ip)) {
            ob_get_clean();
            osc_add_flash_error_message(
                __('<strong>Information!</strong> Your account is disabled due to too much of false login attempts. Please contact the webmaster.',
                    'spamprotection'), 'admin');
        ...
        if ($login_trys < $max_logins && spam_prot::newInstance()->_checkAdminLogin($admin, $data_pass)) {
            ...
            if ($login_trys >= $max_logins) {
                ...
                if (spam_prot::newInstance()->_get('sp_admin_login_inform') == '1') {
                    ob_get_clean();
                    osc_add_flash_error_message(__('<strong>Information!</strong> Your account is disabled due to too much of false login attempts. Please contact the webmaster.',
                        'spamprotection'), 'admin');
                }
            } elseif (empty($login_trys) || $login_trys < $max_logins) {
                if (spam_prot::newInstance()->_get('sp_admin_login_inform') == '1') {
                    ob_get_clean();
                    osc_add_flash_error_message(sprintf(__('<strong>Warning!</strong> Only %d login attempts remaining',
                        'spamprotection'), ($max_logins - $login_trys)), 'admin');
                }
            }
            header('Location: ' . osc_admin_base_url(true) . '?page=login');
            exit;
