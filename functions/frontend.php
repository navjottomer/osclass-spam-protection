...
function sp_check_comment($id)
{
    ...
    if (spam_prot::newInstance()->_get('sp_tor_activate') && spam_prot::newInstance()->_get('sp_tor_comments')
        ...
        ob_get_clean();
        osc_add_flash_error_message(__("Sending contact mails are not allowed because of using the TOR Network. Please disable TOR and try again.",
            "spamprotection"));
        header('Location: ' . osc_base_url());
        exit;
    }
...
function sp_check_user_login()
{
    ...
    if (spam_prot::newInstance()->_get('sp_tor_activate') && spam_prot::newInstance()->_get('sp_tor_login')
        ...
        ob_get_clean();
        osc_add_flash_error_message(__("Login is not allowed because of using the TOR Network. Please disable TOR and try again.",
            "spamprotection"));
        header('Location: ' . osc_base_url());
        exit;
    }
            ...
            if ($logins >= $max_logins) {
                ...
                if (spam_prot::newInstance()->_get('sp_security_login_inform') == '1') {
                    ...
                    osc_add_flash_error_message(sprintf(__('<strong>Warning!</strong> Only %d login attempts remaining',
                        'spamprotection'), ($max_logins - $logins)));
                }
            }
...
function sp_check_user_registrations()
{
    ...
    if (spam_prot::newInstance()->_get('sp_tor_activate') && spam_prot::newInstance()->_get('sp_tor_registration')
        ...
        ob_get_clean();
        osc_add_flash_error_message(__("Registration is not allowed because of using the TOR Network. Please disable TOR and try again.",
            "spamprotection"));
        header('Location: ' . osc_base_url());
        exit;
    }

    if (Params::getParam('action') === 'register_post'
        && spam_prot::newInstance()->_get('sp_check_registrations') >= '2'
    ) {
        if (($email = filter_var($email, FILTER_VALIDATE_EMAIL)) !== false) {
            ...
            if ($check == '2') {
                if (!in_array($domain, $mails)) {
                    $error = sprintf(__('Sorry, but you cannot use this email address: %s',
    ...
    if ($check_ban) {
        ob_get_clean();
        spam_prot::newInstance()
            ->_addGlobalLog('Registration blocked because of existing ban', $email . '/' . $ip, 'System');
    ...
    if ($check_mail == '1' || $check_ip == '1') {
        ...
        if (isset($data_mail) && is_array($data_mail)) {
            ...
            if ($data_freq >= $frequency || $data_conf >= $confidence) {
                ...
                spam_prot::newInstance()
                    ->_addGlobalLog('Registration blocked because listed Email address:', $email, 'StopForumSpam');
        ...
        if (isset($data_ip) && is_array($data_ip)) {
            ...
            if ($data_freq >= $frequency || $data_conf >= $confidence) {
                $ban = spam_prot::newInstance()->_get('sp_autoban_stopforumspam');
                spam_prot::newInstance()
                    ->_addGlobalLog('Registration blocked because listed IP:', $ip, 'StopForumSpam');
...
function sp_block_baduser_contact()
{
    echo '
    <p>'
        . __('Sorry, but you are not allowed to send contact mails. If you think this is a mistake, feel free to contact the support team')
        . '</p>
    <script>
        $(document).ready(function(){
            $("form#contact_form").html("").append("<p>'
        . __('Sorry, but you are not allowed to send contact mails. If you think this is a mistake, feel free to contact the support team')
        . '</p>");
        });
    </script>
