    ...
    foreach ($user as $v) {
        ...
        if (!$checkAdmin || ($checkAdmin && !$isAdmin)) {
            ...
            <tr style="color: ' . $color . '">
                <td>' . (!$isAdmin ? '<input type="checkbox" name="deleteUserID[]" value="' . $v['pk_i_id'] . '" />'
                    : '') . '</td>
                <td>' . $v['pk_i_id'] . '</td>
                <td>' . $v['s_name'] . '</td>
                <td>' . $v['s_email'] . '</td>
...
<a id="deleteNowUnwantedAccounts" class="btn btn-red" data-link="'
        . osc_ajax_plugin_url('spamprotection/functions/searchUser.php')
        . '" style="float: right; margin: 20px 0 0 0;">' . __('Delete Now',
            'spamprotection') . '</a>
<div style="clear: both;"</div>
    ';
}
