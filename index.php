 ...
 *
 * 1.3.4 - Stopwords now shown, if ad was blocked for this reason, Search for duplicates improved, translations
 * corrected
 *
 * 1.4.0 - New selectable method added in search for duplicates, item comment protection added, translations corrected.
 ...
 *
 * 1.5.2 - Added User ban to check ads page, fix problem with clicking id on check ads page, added time range for
 ...
 *
 * 1.6.0 - Redesigned configuration area, admin login protection, import/export for settings and database, plugin
 ...
 *
 * 1.6.3 - Fixed bugs for import, change category for ads, sending emails, and paths for configuration files and some
 * display errors.
 *
 * Added Subject to Mailtemplates, Bad/Trusted user list, Topbar Icon, Themechanger, internal ban if found on
 * StopForumSpam
 *
 ...
 *
 * 1.6.6 - Moved some rows from Table t_user to own table, added fix to copy data and delete not needed rows, changed
 * some stylesheets
 *
 ...
 *
 * 1.7.0 - Added an option to block messages after an ad is posted and added an option to inform the user, that the ad
 * has to be moderated.
 *
...
if ((!osc_is_admin_user_logged_in() && !OC_ADMIN)
    && Params::getParam('page') !== 'sp_activate_account'
    && spam_prot::newInstance()->_get('sp_ipban_activate')
) {
    ...
    if (is_array($ips) && array_key_exists($ip, $ips)) {
...
if ((Params::getParam('sp_check_stopforumspam_mail') || Params::getParam('sp_check_stopforumspam_ip'))
    || (Params::getParam('action') === 'register_post' && $sp->_get('sp_check_registrations') >= '2')
) {
...
if (
    $sp->_get('sp_delete_expired') == '1' || $sp->_get('sp_delete_unactivated') == '1'
